-- phpMyAdmin SQL Dump
-- version 4.1.14
-- http://www.phpmyadmin.net
--
-- Client :  127.0.0.1
-- Généré le :  Mer 08 Mars 2017 à 22:45
-- Version du serveur :  5.6.17
-- Version de PHP :  5.5.12

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8 */;

--
-- Base de données :  `city`
--

-- --------------------------------------------------------

--
-- Structure de la table `admin`
--

CREATE TABLE IF NOT EXISTS `admin` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `last_name` varchar(45) DEFAULT NULL,
  `first_name` varchar(45) DEFAULT NULL,
  `login` varchar(255) NOT NULL,
  `password` varchar(255) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `login` (`login`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Structure de la table `area`
--

CREATE TABLE IF NOT EXISTS `area` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `code` varchar(255) NOT NULL,
  `libelle` varchar(255) NOT NULL,
  `information` varchar(255) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `code_iris` (`code`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=45 ;

--
-- Contenu de la table `area`
--

INSERT INTO `area` (`id`, `code`, `libelle`) VALUES
(1, '543950101', 'Haut-du-Lievre Nord Gentilly'),
(2, '543950102', 'Haut-du-Lievre Tamaris Tilleul Argente'),
(3, '543950103', 'Haut-du-Lievre Blanc Sycomore'),
(4, '543950201', 'Cure d''Air'),
(5, '543950202', 'Boufflers Sud-Ouest Beauregard'),
(6, '543950203', 'Buthegnemont'),
(7, '543950301', 'Colline Scarpone'),
(8, '543950302', 'Colline Boudonville'),
(9, '543950303', 'Albert Ier Verdun Isabey'),
(10, '543950401', 'Preville Patton'),
(11, '543950402', 'Santifontaine'),
(12, '543950403', 'A. France Sud Bertin Nord'),
(13, '543950404', 'Foch Commanderie'),
(14, '543950501', 'Croix de Bourgogne Mouilleron'),
(15, '543950502', 'Mon Desert Nord Jeanne d''Arc Est'),
(16, '543950503', 'Mon Desert Sud Jeanne d''Arc Ouest'),
(17, '543950504', 'Prouve Republique Garenne Nord'),
(18, '543950505', 'Rotonde Kennedy Bellange'),
(19, '543950601', 'Vauban Sud Gebhart Turinaz'),
(20, '543950602', 'Vauban Nord Placieux Parc Sainte-Marie'),
(21, '543950603', 'Haussonville'),
(22, '543950604', 'Leclerc Sud Chevert Exelmans'),
(23, '543950701', 'Leclerc Nord Garenne Sud'),
(24, '543950702', 'Leclerc Sud Saurupt Clemenceau'),
(25, '543950703', 'Oudinot Cimetiere du Sud Doumer'),
(26, '543950801', 'Rene Ii Bonsecours La Madeleine'),
(27, '543950802', 'Pichon Place des Vosges Lebrun Nord'),
(28, '543950803', 'Olry Lobau Remenauville'),
(29, '543950804', 'Canal Marne au Rhin Est M. Brot'),
(30, '543950805', 'Lionnois Hopital Central Maternite'),
(31, '543950901', 'Charles Iii Sud Jardiniers'),
(32, '543950902', 'Gambetta Carmes Faiencerie'),
(33, '543950903', 'Saint-Sebastien Cyffle'),
(34, '543950904', 'Charles Iii Nord Tiercelins St-Georges'),
(35, '543950905', 'Centre Gare Marche'),
(36, '543951001', 'Xxe Corps Sud Austrasie Meurthe'),
(37, '543951002', 'Xxe Corps Nord Oberlin Mac-Mahon'),
(38, '543951003', 'Port Sainte-Catherine Le Jardin d''Eau'),
(39, '543951101', 'Cours Leopold'),
(40, '543951102', 'Pepiniere Vieille Ville'),
(41, '543951201', 'Keller Metz Joli Coeur'),
(42, '543951202', 'Citadelle Sellier C. de Foucauld'),
(43, '543951203', 'V. Mauvais Vayringe Crosne'),
(44, '54395ZZZZ', 'Non localisé à l''iris');

-- --------------------------------------------------------

--
-- Structure de la table `family`
--

CREATE TABLE IF NOT EXISTS `family` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `description` varchar(45) DEFAULT NULL,
  `color` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=2 ;

--
-- Contenu de la table `family`
--

INSERT INTO `family` (`id`, `description`, `color`) VALUES
(0, 'Family 1', '#FFF');

-- --------------------------------------------------------

--
-- Structure de la table `geographical_data`
--

CREATE TABLE IF NOT EXISTS `geographical_data` (
  `id` int(11) NOT NULL,
  `longitude` varchar(45) DEFAULT NULL,
  `latitude` varchar(45) DEFAULT NULL,
  `description` varchar(45) DEFAULT NULL,
  `id_area` int(11) DEFAULT NULL,
  `id_family` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Structure de la table `service`
--

CREATE TABLE IF NOT EXISTS `service` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `title` varchar(45) DEFAULT NULL,
  `id_family` int(11) DEFAULT NULL,
  `information` varchar(255) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=24 ;

--
-- Contenu de la table `service`
--

INSERT INTO `service` (`id`, `title`, `id_family`) VALUES
(1, 'Hypermarché', 0),
(2, 'Supermarché', 0),
(3, 'Grande surface de bricolage', 0),
(4, 'Supérette', 0),
(5, 'Epicerie', 0),
(6, 'Boulangerie', 0),
(7, 'Boucherie charcuterie', 0),
(8, 'Produits surgelés', 0),
(9, 'Poissonnerie', 0),
(10, 'Librairie papeterie journaux', 0),
(11, 'Magasin de vêtements', 0),
(12, 'Magasin d''équipements du foyer', 0),
(13, 'Magasin de chaussures', 0),
(14, 'Magasin d''électroménager et de matériel audio', 0),
(15, 'Magasin de meubles', 0),
(16, 'Magasin d''articles de sports et de loisirs', 0),
(17, 'Magasin de revêtements murs et sols', 0),
(18, 'Droguerie quincaillerie bricolage', 0),
(19, 'Parfumerie', 0),
(20, 'Horlogerie Bijouterie', 0),
(21, 'Fleuriste', 0),
(22, 'Magasin d''optique', 0),
(23, 'Station service', 0);

-- --------------------------------------------------------

--
-- Structure de la table `service_by_area`
--

CREATE TABLE IF NOT EXISTS `service_by_area` (
  `id_service` int(11) NOT NULL,
  `id_area` int(11) NOT NULL,
  `number` int(11) DEFAULT NULL,
  PRIMARY KEY (`id_area`,`id_service`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Contenu de la table `service_by_area`
--

INSERT INTO `service_by_area` (`id_service`, `id_area`, `number`) VALUES
(1, 1, 0),
(2, 1, 0),
(3, 1, 0),
(4, 1, 1),
(5, 1, 1),
(6, 1, 2),
(7, 1, 1),
(8, 1, 0),
(9, 1, 0),
(10, 1, 0),
(11, 1, 0),
(12, 1, 0),
(13, 1, 0),
(14, 1, 0),
(15, 1, 0),
(16, 1, 0),
(17, 1, 0),
(18, 1, 0),
(19, 1, 0),
(20, 1, 0),
(21, 1, 0),
(22, 1, 1),
(23, 1, 0),
(1, 2, 0),
(2, 2, 0),
(3, 2, 0),
(4, 2, 0),
(5, 2, 0),
(6, 2, 0),
(7, 2, 0),
(8, 2, 0),
(9, 2, 0),
(10, 2, 1),
(11, 2, 1),
(12, 2, 0),
(13, 2, 0),
(14, 2, 0),
(15, 2, 0),
(16, 2, 0),
(17, 2, 0),
(18, 2, 0),
(19, 2, 0),
(20, 2, 0),
(21, 2, 0),
(22, 2, 0),
(23, 2, 0),
(1, 3, 0),
(2, 3, 0),
(3, 3, 0),
(4, 3, 0),
(5, 3, 0),
(6, 3, 0),
(7, 3, 0),
(8, 3, 0),
(9, 3, 0),
(10, 3, 0),
(11, 3, 0),
(12, 3, 0),
(13, 3, 0),
(14, 3, 0),
(15, 3, 0),
(16, 3, 0),
(17, 3, 0),
(18, 3, 0),
(19, 3, 0),
(20, 3, 0),
(21, 3, 0),
(22, 3, 0),
(23, 3, 0),
(1, 4, 0),
(2, 4, 0),
(3, 4, 0),
(4, 4, 0),
(5, 4, 0),
(6, 4, 0),
(7, 4, 0),
(8, 4, 0),
(9, 4, 0),
(10, 4, 0),
(11, 4, 1),
(12, 4, 0),
(13, 4, 0),
(14, 4, 0),
(15, 4, 0),
(16, 4, 0),
(17, 4, 0),
(18, 4, 0),
(19, 4, 0),
(20, 4, 0),
(21, 4, 0),
(22, 4, 0),
(23, 4, 0),
(1, 5, 0),
(2, 5, 0),
(3, 5, 0),
(4, 5, 0),
(5, 5, 1),
(6, 5, 0),
(7, 5, 0),
(8, 5, 0),
(9, 5, 0),
(10, 5, 0),
(11, 5, 1),
(12, 5, 0),
(13, 5, 0),
(14, 5, 0),
(15, 5, 0),
(16, 5, 0),
(17, 5, 0),
(18, 5, 0),
(19, 5, 1),
(20, 5, 0),
(21, 5, 0),
(22, 5, 0),
(23, 5, 0),
(1, 6, 0),
(2, 6, 0),
(3, 6, 0),
(4, 6, 0),
(5, 6, 0),
(6, 6, 1),
(7, 6, 0),
(8, 6, 0),
(9, 6, 0),
(10, 6, 0),
(11, 6, 1),
(12, 6, 0),
(13, 6, 0),
(14, 6, 0),
(15, 6, 0),
(16, 6, 0),
(17, 6, 0),
(18, 6, 1),
(19, 6, 0),
(20, 6, 0),
(21, 6, 1),
(22, 6, 0),
(23, 6, 0),
(1, 7, 0),
(2, 7, 0),
(3, 7, 0),
(4, 7, 0),
(5, 7, 0),
(6, 7, 1),
(7, 7, 0),
(8, 7, 0),
(9, 7, 0),
(10, 7, 0),
(11, 7, 1),
(12, 7, 0),
(13, 7, 0),
(14, 7, 0),
(15, 7, 0),
(16, 7, 0),
(17, 7, 0),
(18, 7, 0),
(19, 7, 1),
(20, 7, 0),
(21, 7, 0),
(22, 7, 0),
(23, 7, 0),
(1, 8, 0),
(2, 8, 0),
(3, 8, 0),
(4, 8, 0),
(5, 8, 0),
(6, 8, 2),
(7, 8, 0),
(8, 8, 0),
(9, 8, 0),
(10, 8, 1),
(11, 8, 0),
(12, 8, 0),
(13, 8, 0),
(14, 8, 0),
(15, 8, 0),
(16, 8, 0),
(17, 8, 0),
(18, 8, 0),
(19, 8, 0),
(20, 8, 0),
(21, 8, 0),
(22, 8, 0),
(23, 8, 0),
(1, 9, 0),
(2, 9, 0),
(3, 9, 0),
(4, 9, 0),
(5, 9, 1),
(6, 9, 1),
(7, 9, 0),
(8, 9, 0),
(9, 9, 0),
(10, 9, 1),
(11, 9, 0),
(12, 9, 0),
(13, 9, 0),
(14, 9, 1),
(15, 9, 0),
(16, 9, 0),
(17, 9, 0),
(18, 9, 0),
(19, 9, 0),
(20, 9, 0),
(21, 9, 0),
(22, 9, 0),
(23, 9, 2),
(1, 10, 0),
(2, 10, 2),
(3, 10, 0),
(4, 10, 0),
(5, 10, 0),
(6, 10, 0),
(7, 10, 0),
(8, 10, 0),
(9, 10, 0),
(10, 10, 0),
(11, 10, 1),
(12, 10, 1),
(13, 10, 0),
(14, 10, 0),
(15, 10, 0),
(16, 10, 0),
(17, 10, 1),
(18, 10, 1),
(19, 10, 0),
(20, 10, 0),
(21, 10, 1),
(22, 10, 0),
(23, 10, 1),
(1, 11, 0),
(2, 11, 0),
(3, 11, 0),
(4, 11, 0),
(5, 11, 0),
(6, 11, 1),
(7, 11, 1),
(8, 11, 0),
(9, 11, 0),
(10, 11, 0),
(11, 11, 0),
(12, 11, 0),
(13, 11, 0),
(14, 11, 0),
(15, 11, 0),
(16, 11, 0),
(17, 11, 0),
(18, 11, 0),
(19, 11, 0),
(20, 11, 0),
(21, 11, 0),
(22, 11, 1),
(23, 11, 0),
(1, 12, 0),
(2, 12, 1),
(3, 12, 0),
(4, 12, 0),
(5, 12, 0),
(6, 12, 3),
(7, 12, 1),
(8, 12, 1),
(9, 12, 0),
(10, 12, 0),
(11, 12, 0),
(12, 12, 0),
(13, 12, 0),
(14, 12, 0),
(15, 12, 0),
(16, 12, 0),
(17, 12, 0),
(18, 12, 0),
(19, 12, 0),
(20, 12, 0),
(21, 12, 1),
(22, 12, 1),
(23, 12, 0),
(1, 13, 0),
(2, 13, 0),
(3, 13, 0),
(4, 13, 2),
(5, 13, 4),
(6, 13, 3),
(7, 13, 3),
(8, 13, 0),
(9, 13, 0),
(10, 13, 1),
(11, 13, 0),
(12, 13, 0),
(13, 13, 0),
(14, 13, 0),
(15, 13, 2),
(16, 13, 0),
(17, 13, 0),
(18, 13, 0),
(19, 13, 1),
(20, 13, 0),
(21, 13, 0),
(22, 13, 1),
(23, 13, 0),
(1, 14, 0),
(2, 14, 2),
(3, 14, 0),
(4, 14, 0),
(5, 14, 1),
(6, 14, 3),
(7, 14, 0),
(8, 14, 0),
(9, 14, 0),
(10, 14, 0),
(11, 14, 1),
(12, 14, 1),
(13, 14, 0),
(14, 14, 1),
(15, 14, 1),
(16, 14, 0),
(17, 14, 0),
(18, 14, 0),
(19, 14, 0),
(20, 14, 1),
(21, 14, 2),
(22, 14, 0),
(23, 14, 0),
(1, 15, 0),
(2, 15, 0),
(3, 15, 0),
(4, 15, 0),
(5, 15, 0),
(6, 15, 1),
(7, 15, 1),
(8, 15, 0),
(9, 15, 0),
(10, 15, 0),
(11, 15, 0),
(12, 15, 0),
(13, 15, 0),
(14, 15, 0),
(15, 15, 0),
(16, 15, 0),
(17, 15, 0),
(18, 15, 0),
(19, 15, 1),
(20, 15, 1),
(21, 15, 1),
(22, 15, 0),
(23, 15, 0),
(1, 16, 0),
(2, 16, 0),
(3, 16, 0),
(4, 16, 0),
(5, 16, 0),
(6, 16, 2),
(7, 16, 0),
(8, 16, 0),
(9, 16, 0),
(10, 16, 0),
(11, 16, 1),
(12, 16, 0),
(13, 16, 0),
(14, 16, 0),
(15, 16, 0),
(16, 16, 0),
(17, 16, 0),
(18, 16, 0),
(19, 16, 0),
(20, 16, 0),
(21, 16, 1),
(22, 16, 0),
(23, 16, 0),
(1, 17, 0),
(2, 17, 0),
(3, 17, 0),
(4, 17, 1),
(5, 17, 1),
(6, 17, 2),
(7, 17, 1),
(8, 17, 0),
(9, 17, 0),
(10, 17, 0),
(11, 17, 0),
(12, 17, 0),
(13, 17, 0),
(14, 17, 0),
(15, 17, 0),
(16, 17, 0),
(17, 17, 0),
(18, 17, 1),
(19, 17, 0),
(20, 17, 0),
(21, 17, 1),
(22, 17, 0),
(23, 17, 1),
(1, 18, 0),
(2, 18, 0),
(3, 18, 0),
(4, 18, 0),
(5, 18, 0),
(6, 18, 0),
(7, 18, 0),
(8, 18, 0),
(9, 18, 0),
(10, 18, 0),
(11, 18, 0),
(12, 18, 0),
(13, 18, 0),
(14, 18, 0),
(15, 18, 0),
(16, 18, 0),
(17, 18, 0),
(18, 18, 0),
(19, 18, 0),
(20, 18, 0),
(21, 18, 0),
(22, 18, 0),
(23, 18, 0),
(1, 19, 0),
(2, 19, 0),
(3, 19, 0),
(4, 19, 0),
(5, 19, 1),
(6, 19, 5),
(7, 19, 5),
(8, 19, 0),
(9, 19, 0),
(10, 19, 2),
(11, 19, 1),
(12, 19, 0),
(13, 19, 0),
(14, 19, 0),
(15, 19, 0),
(16, 19, 0),
(17, 19, 0),
(18, 19, 0),
(19, 19, 0),
(20, 19, 0),
(21, 19, 0),
(22, 19, 1),
(23, 19, 0),
(1, 20, 0),
(2, 20, 1),
(3, 20, 0),
(4, 20, 0),
(5, 20, 0),
(6, 20, 3),
(7, 20, 1),
(8, 20, 0),
(9, 20, 0),
(10, 20, 2),
(11, 20, 1),
(12, 20, 0),
(13, 20, 0),
(14, 20, 0),
(15, 20, 0),
(16, 20, 2),
(17, 20, 0),
(18, 20, 0),
(19, 20, 0),
(20, 20, 0),
(21, 20, 0),
(22, 20, 0),
(23, 20, 0),
(1, 21, 0),
(2, 21, 1),
(3, 21, 0),
(4, 21, 0),
(5, 21, 0),
(6, 21, 0),
(7, 21, 1),
(8, 21, 0),
(9, 21, 0),
(10, 21, 0),
(11, 21, 1),
(12, 21, 0),
(13, 21, 0),
(14, 21, 0),
(15, 21, 0),
(16, 21, 0),
(17, 21, 0),
(18, 21, 0),
(19, 21, 0),
(20, 21, 0),
(21, 21, 0),
(22, 21, 0),
(23, 21, 0),
(1, 22, 0),
(2, 22, 0),
(3, 22, 0),
(4, 22, 1),
(5, 22, 0),
(6, 22, 3),
(7, 22, 0),
(8, 22, 0),
(9, 22, 0),
(10, 22, 0),
(11, 22, 0),
(12, 22, 0),
(13, 22, 0),
(14, 22, 0),
(15, 22, 2),
(16, 22, 1),
(17, 22, 0),
(18, 22, 0),
(19, 22, 0),
(20, 22, 0),
(21, 22, 0),
(22, 22, 0),
(23, 22, 0),
(1, 23, 0),
(2, 23, 0),
(3, 23, 0),
(4, 23, 0),
(5, 23, 4),
(6, 23, 2),
(7, 23, 0),
(8, 23, 0),
(9, 23, 0),
(10, 23, 0),
(11, 23, 0),
(12, 23, 0),
(13, 23, 0),
(14, 23, 0),
(15, 23, 0),
(16, 23, 0),
(17, 23, 0),
(18, 23, 1),
(19, 23, 0),
(20, 23, 0),
(21, 23, 0),
(22, 23, 0),
(23, 23, 0),
(1, 24, 0),
(2, 24, 0),
(3, 24, 0),
(4, 24, 0),
(5, 24, 0),
(6, 24, 1),
(7, 24, 0),
(8, 24, 0),
(9, 24, 0),
(10, 24, 0),
(11, 24, 0),
(12, 24, 0),
(13, 24, 0),
(14, 24, 0),
(15, 24, 0),
(16, 24, 0),
(17, 24, 0),
(18, 24, 0),
(19, 24, 0),
(20, 24, 0),
(21, 24, 0),
(22, 24, 0),
(23, 24, 0),
(1, 25, 0),
(2, 25, 0),
(3, 25, 1),
(4, 25, 0),
(5, 25, 1),
(6, 25, 2),
(7, 25, 0),
(8, 25, 0),
(9, 25, 0),
(10, 25, 0),
(11, 25, 0),
(12, 25, 0),
(13, 25, 0),
(14, 25, 0),
(15, 25, 0),
(16, 25, 0),
(17, 25, 0),
(18, 25, 0),
(19, 25, 0),
(20, 25, 1),
(21, 25, 0),
(22, 25, 0),
(23, 25, 0),
(1, 26, 0),
(2, 26, 0),
(3, 26, 0),
(4, 26, 0),
(5, 26, 1),
(6, 26, 2),
(7, 26, 0),
(8, 26, 0),
(9, 26, 0),
(10, 26, 0),
(11, 26, 0),
(12, 26, 0),
(13, 26, 0),
(14, 26, 0),
(15, 26, 0),
(16, 26, 0),
(17, 26, 0),
(18, 26, 0),
(19, 26, 0),
(20, 26, 0),
(21, 26, 1),
(22, 26, 0),
(23, 26, 0),
(1, 27, 0),
(2, 27, 2),
(3, 27, 0),
(4, 27, 0),
(5, 27, 1),
(6, 27, 2),
(7, 27, 0),
(8, 27, 0),
(9, 27, 0),
(10, 27, 1),
(11, 27, 0),
(12, 27, 0),
(13, 27, 0),
(14, 27, 1),
(15, 27, 0),
(16, 27, 0),
(17, 27, 0),
(18, 27, 0),
(19, 27, 0),
(20, 27, 0),
(21, 27, 0),
(22, 27, 0),
(23, 27, 0),
(1, 28, 1),
(2, 28, 0),
(3, 28, 2),
(4, 28, 0),
(5, 28, 0),
(6, 28, 0),
(7, 28, 1),
(8, 28, 0),
(9, 28, 0),
(10, 28, 1),
(11, 28, 0),
(12, 28, 0),
(13, 28, 0),
(14, 28, 1),
(15, 28, 1),
(16, 28, 0),
(17, 28, 0),
(18, 28, 1),
(19, 28, 0),
(20, 28, 1),
(21, 28, 0),
(22, 28, 1),
(23, 28, 1),
(1, 29, 1),
(2, 29, 2),
(3, 29, 0),
(4, 29, 1),
(5, 29, 1),
(6, 29, 0),
(7, 29, 1),
(8, 29, 0),
(9, 29, 0),
(10, 29, 0),
(11, 29, 0),
(12, 29, 0),
(13, 29, 0),
(14, 29, 0),
(15, 29, 0),
(16, 29, 0),
(17, 29, 0),
(18, 29, 2),
(19, 29, 0),
(20, 29, 0),
(21, 29, 0),
(22, 29, 0),
(23, 29, 0),
(1, 30, 0),
(2, 30, 0),
(3, 30, 0),
(4, 30, 0),
(5, 30, 1),
(6, 30, 0),
(7, 30, 0),
(8, 30, 0),
(9, 30, 0),
(10, 30, 1),
(11, 30, 0),
(12, 30, 0),
(13, 30, 0),
(14, 30, 0),
(15, 30, 0),
(16, 30, 0),
(17, 30, 0),
(18, 30, 0),
(19, 30, 0),
(20, 30, 0),
(21, 30, 0),
(22, 30, 0),
(23, 30, 0),
(1, 31, 0),
(2, 31, 0),
(3, 31, 0),
(4, 31, 0),
(5, 31, 3),
(6, 31, 2),
(7, 31, 0),
(8, 31, 0),
(9, 31, 0),
(10, 31, 0),
(11, 31, 0),
(12, 31, 0),
(13, 31, 0),
(14, 31, 0),
(15, 31, 2),
(16, 31, 0),
(17, 31, 0),
(18, 31, 0),
(19, 31, 0),
(20, 31, 0),
(21, 31, 0),
(22, 31, 0),
(23, 31, 0),
(1, 32, 0),
(2, 32, 2),
(3, 32, 1),
(4, 32, 0),
(5, 32, 5),
(6, 32, 9),
(7, 32, 2),
(8, 32, 0),
(9, 32, 0),
(10, 32, 8),
(11, 32, 91),
(12, 32, 7),
(13, 32, 18),
(14, 32, 2),
(15, 32, 2),
(16, 32, 3),
(17, 32, 0),
(18, 32, 0),
(19, 32, 11),
(20, 32, 11),
(21, 32, 0),
(22, 32, 16),
(23, 32, 0),
(1, 33, 1),
(2, 33, 1),
(3, 33, 0),
(4, 33, 0),
(5, 33, 1),
(6, 33, 2),
(7, 33, 0),
(8, 33, 1),
(9, 33, 0),
(10, 33, 7),
(11, 33, 57),
(12, 33, 3),
(13, 33, 8),
(14, 33, 2),
(15, 33, 5),
(16, 33, 5),
(17, 33, 0),
(18, 33, 0),
(19, 33, 10),
(20, 33, 7),
(21, 33, 0),
(22, 33, 8),
(23, 33, 0),
(1, 34, 0),
(2, 34, 0),
(3, 34, 0),
(4, 34, 1),
(5, 34, 3),
(6, 34, 8),
(7, 34, 3),
(8, 34, 0),
(9, 34, 0),
(10, 34, 2),
(11, 34, 25),
(12, 34, 3),
(13, 34, 5),
(14, 34, 0),
(15, 34, 4),
(16, 34, 1),
(17, 34, 0),
(18, 34, 0),
(19, 34, 0),
(20, 34, 2),
(21, 34, 3),
(22, 34, 0),
(23, 34, 0),
(1, 35, 0),
(2, 35, 0),
(3, 35, 0),
(4, 35, 2),
(5, 35, 4),
(6, 35, 5),
(7, 35, 7),
(8, 35, 0),
(9, 35, 1),
(10, 35, 7),
(11, 35, 42),
(12, 35, 4),
(13, 35, 11),
(14, 35, 2),
(15, 35, 1),
(16, 35, 2),
(17, 35, 0),
(18, 35, 1),
(19, 35, 6),
(20, 35, 4),
(21, 35, 4),
(22, 35, 3),
(23, 35, 0),
(1, 36, 0),
(2, 36, 1),
(3, 36, 0),
(4, 36, 0),
(5, 36, 0),
(6, 36, 5),
(7, 36, 0),
(8, 36, 0),
(9, 36, 0),
(10, 36, 0),
(11, 36, 0),
(12, 36, 0),
(13, 36, 0),
(14, 36, 0),
(15, 36, 1),
(16, 36, 1),
(17, 36, 0),
(18, 36, 0),
(19, 36, 1),
(20, 36, 0),
(21, 36, 0),
(22, 36, 0),
(23, 36, 1),
(1, 37, 0),
(2, 37, 0),
(3, 37, 1),
(4, 37, 0),
(5, 37, 3),
(6, 37, 3),
(7, 37, 0),
(8, 37, 0),
(9, 37, 0),
(10, 37, 0),
(11, 37, 0),
(12, 37, 1),
(13, 37, 0),
(14, 37, 1),
(15, 37, 0),
(16, 37, 1),
(17, 37, 0),
(18, 37, 0),
(19, 37, 0),
(20, 37, 0),
(21, 37, 0),
(22, 37, 0),
(23, 37, 0),
(1, 38, 0),
(2, 38, 0),
(3, 38, 0),
(4, 38, 1),
(5, 38, 0),
(6, 38, 0),
(7, 38, 0),
(8, 38, 0),
(9, 38, 0),
(10, 38, 1),
(11, 38, 0),
(12, 38, 0),
(13, 38, 0),
(14, 38, 0),
(15, 38, 0),
(16, 38, 0),
(17, 38, 0),
(18, 38, 0),
(19, 38, 0),
(20, 38, 0),
(21, 38, 0),
(22, 38, 0),
(23, 38, 0),
(1, 39, 0),
(2, 39, 0),
(3, 39, 0),
(4, 39, 2),
(5, 39, 3),
(6, 39, 4),
(7, 39, 0),
(8, 39, 0),
(9, 39, 0),
(10, 39, 0),
(11, 39, 7),
(12, 39, 0),
(13, 39, 2),
(14, 39, 1),
(15, 39, 0),
(16, 39, 2),
(17, 39, 0),
(18, 39, 0),
(19, 39, 0),
(20, 39, 0),
(21, 39, 1),
(22, 39, 0),
(23, 39, 0),
(1, 40, 0),
(2, 40, 0),
(3, 40, 0),
(4, 40, 0),
(5, 40, 2),
(6, 40, 4),
(7, 40, 2),
(8, 40, 0),
(9, 40, 0),
(10, 40, 6),
(11, 40, 3),
(12, 40, 7),
(13, 40, 2),
(14, 40, 1),
(15, 40, 2),
(16, 40, 0),
(17, 40, 0),
(18, 40, 1),
(19, 40, 0),
(20, 40, 2),
(21, 40, 4),
(22, 40, 1),
(23, 40, 0),
(1, 41, 0),
(2, 41, 0),
(3, 41, 0),
(4, 41, 0),
(5, 41, 3),
(6, 41, 3),
(7, 41, 0),
(8, 41, 1),
(9, 41, 0),
(10, 41, 1),
(11, 41, 1),
(12, 41, 0),
(13, 41, 0),
(14, 41, 0),
(15, 41, 0),
(16, 41, 0),
(17, 41, 0),
(18, 41, 0),
(19, 41, 0),
(20, 41, 0),
(21, 41, 0),
(22, 41, 1),
(23, 41, 0),
(1, 42, 0),
(2, 42, 0),
(3, 42, 0),
(4, 42, 0),
(5, 42, 1),
(6, 42, 2),
(7, 42, 1),
(8, 42, 0),
(9, 42, 0),
(10, 42, 0),
(11, 42, 0),
(12, 42, 0),
(13, 42, 0),
(14, 42, 0),
(15, 42, 0),
(16, 42, 0),
(17, 42, 0),
(18, 42, 0),
(19, 42, 1),
(20, 42, 0),
(21, 42, 1),
(22, 42, 1),
(23, 42, 0),
(1, 43, 0),
(2, 43, 0),
(3, 43, 0),
(4, 43, 0),
(5, 43, 0),
(6, 43, 3),
(7, 43, 0),
(8, 43, 0),
(9, 43, 0),
(10, 43, 0),
(11, 43, 2),
(12, 43, 0),
(13, 43, 0),
(14, 43, 0),
(15, 43, 0),
(16, 43, 1),
(17, 43, 0),
(18, 43, 0),
(19, 43, 0),
(20, 43, 0),
(21, 43, 1),
(22, 43, 1),
(23, 43, 0),
(1, 44, 0),
(2, 44, 0),
(3, 44, 0),
(4, 44, 0),
(5, 44, 0),
(6, 44, 0),
(7, 44, 0),
(8, 44, 0),
(9, 44, 0),
(10, 44, 0),
(11, 44, 0),
(12, 44, 0),
(13, 44, 0),
(14, 44, 0),
(15, 44, 0),
(16, 44, 0),
(17, 44, 0),
(18, 44, 0),
(19, 44, 0),
(20, 44, 0),
(21, 44, 0),
(22, 44, 0),
(23, 44, 1);

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
